...
class Track():
    def __init__(self, index):
        self.track_image = image.load(
            os.path.join('images', f'track{index}.png'))
        self.track_overlay_image = image.load(
            os.path.join('images', f'track{index}-overlay.png'))
        ...
        map = [1 if b == (75, 75, 75, 255)
               else 0 for b in itertools.batched(pixels, 4)]
